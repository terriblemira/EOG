...
def run_calibration(eog_thread, window, font, clock, WIDTH, HEIGHT): # variable eog_thread in calibration.py = variable eog_thread in test.py (main "grabs" eog_reader from calibration and uses it as eog_thread)
    ...
    print(f"Calibration window dimensions: {actual_width}x{actual_height}")

    # Use the actual dimensions
    center_pos = [actual_width // 2, actual_height // 2]
    ...
    for direction, is_horizontal in [("left", True), ("right", True), ("up", False), ("down", False)]:
        try:
            ...
            if len(abs_signals) > 0:
                threshold = np.percentile(abs_signals, 75) if not is_horizontal else np.percentile(abs_signals, 85)
